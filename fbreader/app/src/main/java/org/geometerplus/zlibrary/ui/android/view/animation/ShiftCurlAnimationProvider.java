...
import org.geometerplus.zlibrary.ui.android.view.animation.AnimationProvider.Mode;

public final class ShiftCurlAnimationProvider extends ShiftAnimationProvider {
	...
	public ShiftCurlAnimationProvider(BitmapManager bitmapManager) {
		super(bitmapManager);
	...
	protected void drawInternal(Canvas canvas) {
		...
		if(view.getPage(PageIndex.current).PAGENO % 2 == 1) {
		...
		if(isCurl) {
			drawCurlInternal(canvas);
		} else {
			super.drawInternal(canvas);
		}
	}
	
	private void drawCurlInternal(Canvas canvas) {
		if (myUseCanvasHack) {
	...
	protected void startAnimatedScrollingInternal(int speed) {
		if(isCurl) {
			...
			super.startAnimatedScrollingInternal(speed);
		}
	}
	...
	protected void setupAnimatedScrollingStart(Integer x, Integer y) {
		...
		if(isCurl) {
			setupCurlAnimatedScrollingStart(x, y);
		} else {
	...
	protected void setupCurlAnimatedScrollingStart(Integer x, Integer y) {
		if (x == null || y == null) {
			if (myDirection.IsHorizontal) {
