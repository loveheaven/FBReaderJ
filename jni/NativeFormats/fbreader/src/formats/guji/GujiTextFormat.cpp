...
GujiTextFormat::GujiTextFormat(const ZLFile &file) :
...
//	CreateContentsTableOption(FBCategoryKey::BOOKS, file.path(), OPTION_CreateContentsTable, false) {
	...
	myBreakType(1),
	myIgnoredIndent(1),
	myEmptyLinesBeforeNewSection(100),
...
void GujiTextFormatDetector::detect(ZLInputStream &stream, GujiTextFormat &format) {
	if (!stream.open()) {
		return;
	}
	...
	{
		int breakType = 0;
		breakType |= GujiTextFormat::BREAK_PARAGRAPH_AT_EMPTY_LINE;
		if (stringsWithLengthLessThan81Counter < 0.3 * nonEmptyLineCounter) {
			breakType |= GujiTextFormat::BREAK_PARAGRAPH_AT_NEW_LINE;
		} else {
			breakType |= GujiTextFormat::BREAK_PARAGRAPH_AT_LINE_WITH_INDENT;
		}
...

	{
		...
		format.myEmptyLinesBeforeNewSection = 100;//(emptyLinesBeforeNewSection);
...
int BreakTypeOptionEntry::initialIndex() const {
	switch (myBreakTypeOption.value()) {
		case GujiTextFormat::BREAK_PARAGRAPH_AT_NEW_LINE:
			return 0;
		case GujiTextFormat::BREAK_PARAGRAPH_AT_EMPTY_LINE:
			return 1;
		case GujiTextFormat::BREAK_PARAGRAPH_AT_EMPTY_LINE | GujiTextFormat::BREAK_PARAGRAPH_AT_LINE_WITH_INDENT:
		default:
			return 2;
	}
}
...
void BreakTypeOptionEntry::onAccept(const std::string &value) {
	if (value == values()[0]) {
		myBreakTypeOption.setValue(GujiTextFormat::BREAK_PARAGRAPH_AT_NEW_LINE);
	} else if (value == values()[1]) {
		myBreakTypeOption.setValue(GujiTextFormat::BREAK_PARAGRAPH_AT_EMPTY_LINE);
	} else if (value == values()[2]) {
		myBreakTypeOption.setValue(GujiTextFormat::BREAK_PARAGRAPH_AT_EMPTY_LINE | GujiTextFormat::BREAK_PARAGRAPH_AT_LINE_WITH_INDENT);
	}
}
