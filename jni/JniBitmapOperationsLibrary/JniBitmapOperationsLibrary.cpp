...
extern "C"
  ...
  JNIEXPORT jint JNICALL Java_com_jni_bitmap_1operations_JniBitmapHolder_jniGetPixel(JNIEnv * env, jobject obj, jobject handle, uint32_t pos);
...
JNIEXPORT void JNICALL Java_com_jni_bitmap_1operations_JniBitmapHolder_jniRotateBitmapCcw90(JNIEnv * env, jobject obj, jobject handle)
  ...
  for (int x = 0; x < bitmapInfo.height; ++x)
    for (int y = bitmapInfo.width - 1; y >= 0; --y)
      ...
      uint32_t pixel = previousData[whereToGet++];
      newBitmapPixels[bitmapInfo.height * y + x] = pixel;
      }
  delete[] previousData;
  jniBitmap->_storedBitmapPixels = newBitmapPixels;
  uint32_t temp = bitmapInfo.width;
...
JNIEXPORT jint JNICALL Java_com_jni_bitmap_1operations_JniBitmapHolder_jniGetPixel(JNIEnv * env, jobject obj, jobject handle, uint32_t pos)
{
	...
	if(bitmap == NULL)
		return 0;
	return (jint)(bitmap->_storedBitmapPixels[pos]);
}
